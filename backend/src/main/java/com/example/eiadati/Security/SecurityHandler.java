package com.example.eiadati.Security;
...
public class SecurityHandler {
  ...
  private void checkJWT(String bearer, String tipo) throws Exception {
    if (
      bearer == null || !bearer.substring(0, 6).equals("Bearer")
    ...
    if (data.getExpiration().before(instant)) throw new Exception(
      "Token expirado"
    );
    ...
    if (role == null) throw new Exception("Usuário não permitido");
  }
  ...
  public String createJWT(User user) {
    ...
    String token = Jwts
      ...
      .setExpiration(new Date(System.currentTimeMillis() + (60000 * 360)))
      .compact();
